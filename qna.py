...
class QuestionAnswer:
  ...
  """
  def __init__(self,  context=[], answers=[], preprocessed_data=None, vectorizer_name="tfidf"):
    """
      Initialize inputs, context, answers and compute questions tfidf_matrix for inline question matching
    """
    vectorizers = {"tfidf":TfidfVectorizer(), "cv":CountVectorizer()}
    self.vectorizer =vectorizers[vectorizer_name]  # initialize tf idf vectorizer
  ...
  def get_answer_percontext(self, question, min_score=0.28, tfidf_vector=None, tfidf_matrix=None):
      ...
      target_idx = np.argsort(cos_sim)[-1]

      # match output to answer
      if min_score is not None:
        target_idxs = np.where(cos_sim>=min_score)[0]
